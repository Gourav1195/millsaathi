...
Start the Worker on `:8787` for API work and use `npm run dev:web` for isolated frontend work.
...
| `/app/documents` | `components/documents-app.tsx` | documents | upload/void/download/print |
...
`npm run build:worker` produces `dist/web` by preserving the legacy marketing/demo assets and
overlaying the static Next export. It also preserves the legacy operational UI at `/legacy-app`
until the remaining React create/edit/void forms have parity. `npm run deploy` builds that bundle
and deploys the Worker. Verify the production smoke flow before retiring that fallback.
