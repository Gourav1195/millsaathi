...
export function ProcessCatalog({
  ...
  editorTypeId: controlledEditorTypeId,
  onEditorTypeIdChange,
}: {
  open: boolean;
  onClose: () => void;
  ...
  onEditorTypeIdChange?: (id: string | null) => void;
}) {
  const [internalEditorTypeId, setInternalEditorTypeId] = useState<string | null>(null);
  const editingTypeId = onEditorTypeIdChange ? (controlledEditorTypeId ?? null) : internalEditorTypeId;
  const setEditingTypeId = onEditorTypeIdChange ?? setInternalEditorTypeId;
  ...
  useEffect(() => {
    if (!open) {
      ...
      setNewName('');
      setNewDescription('');
      if (!editingTypeId) setError(null);
    }
  }, [open, editingTypeId]);

  useEffect(() => {
    if (!editingType) return;
    setDefaultUnit(editingType.default_unit ?? preferredUnit);
    setDefaultGodownId(editingType.default_destination_godown_id ?? '');
    setDraftLines((editingType.template_lines ?? []).map((line) => toDraftLine(line, preferredUnit)));
    setError(null);
  }, [editingType, preferredUnit]);

  useEffect(() => {
    const dialog = editorDialogRef.current;
    if (!dialog) return;
    if (editingType && !dialog.open) dialog.showModal();
    if (!editingType && dialog.open) dialog.close();
  }, [editingType]);
  ...
  if (!open && !editingType) return null;
  ...
  return (
    <>
      ...
      <dialog
        ref={editorDialogRef}
        className="app-dialog process-editor-dialog"
        onClose={() => {
      ...
      >
        {editingType ? (
          <>
            ...
            <div className="process-editor-dialog-body">
              ...
              <div className="template-rows inline-template-rows">
                {draftLines.length ? draftLines.map((line, index) => (
                  ...
                  <p className="muted process-editor-empty">No flow lines yet. Add one to configure this process template.</p>
                )}
              </div>
