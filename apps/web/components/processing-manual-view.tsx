...
export function ProcessingManualView({
  session,
  types,
  catalogTypes,
  chains,
  ...
  catalogTypes: CatalogProcessType[];
  ...
  useEffect(() => {
    setSelectedTypeId((current) => (types.some((type) => type.id === current) ? current : types[0]?.id ?? ''));
  }, [types]);
  ...
  useEffect(() => {
    void loadRuns().catch((cause: unknown) => onError(cause instanceof Error ? cause.message : 'Could not load process runs'));
  }, [onError]);
  ...
  useEffect(() => {
    ...
    void loadWorkspace(selectedTypeId).catch((cause: unknown) => onError(cause instanceof Error ? cause.message : 'Could not load workspace'));
  }, [selectedTypeId, loadWorkspace, onError]);
  ...
  function onDragEnd(event: DragEndEvent) {
    if (event.over?.id === 'process-inputs') addLot(String(event.active.id));
  }
  ...
  function updateOutput(index: number, quantity: string) {
    setOutputs((current) => current.map((output, i) => i === index ? { ...output, quantity } : output));
  }
  ...
  async function postRun() {
    ...
    if (inputs.some((input) => Number(input.quantity) <= 0)) { onError('Enter a positive quantity for every selected input lot.'); return; }
    if (outputs.some((output) => output.required && Number(output.quantity) <= 0)) { onError('Enter an actual quantity for every required output.'); return; }
    setPosting(true);
    onError(null);
    const lines = [
    ...
    try {
      ...
      if (!response.ok) throw new Error(body.error ?? 'Could not post process run');
      setInputs([]);
      setOutputs((current) => current.map((output) => ({ ...output, quantity: '' })));
      await loadRuns();
    } catch (cause) {
      onError(cause instanceof Error ? cause.message : 'Could not post process run');
    } finally {
      setPosting(false);
    }
  }
  ...
  async function voidRun(run: ProcessRun) {
    ...
    try {
      onError(null);
      const response = await fetch(`/api/process-runs/${run.id}/void`, { method: 'POST', credentials: 'include' });
      const body = await response.json();
      if (!response.ok) throw new Error(body.error ?? 'Could not void process run');
      await loadRuns();
    } catch (cause) {
      onError(cause instanceof Error ? cause.message : 'Could not void process run');
    }
  }
  ...
  return (
    <>
      <div className="process-toolbar">
        <div className="process-toolbar-actions">
          ...
          {canManageOrg && selectedTypeId ? (
            <Button type="button" className="secondary process-catalog-toggle" onClick={() => setEditorTypeId(selectedTypeId)}>
              Edit process
            </Button>
          ) : null}
          {canManageOrg ? (
      ...
      {!workspace && <p className="muted">Loading processing workspace…</p>}
      {workspace && (
        <DndContext onDragEnd={onDragEnd}>
          <div className="process-flow">
            <section className="process-column process-column--materials">
              <h2 className="process-column-title">Available materials <span>{workspace.lots.length}</span></h2>
              <div className="process-materials">
                {workspace.lots.length ? workspace.lots.map((lot) => (
                  <DraggableLot key={lot.id} lot={lot} selected={inputs.some((input) => input.id === lot.id)} />
                )) : <p className="muted">No available lots.</p>}
              </div>
            </section>
      ...
      {workspace && (
        <div className={`balance${balance.differenceKg < 0 || hasInputErrors ? ' warn' : ''}`}>
          Input: <strong>{(balance.inputKg / 100).toFixed(3)} qtl</strong> · Accounted: <strong>{(balance.outputKg / 100).toFixed(3)} qtl</strong> · {balance.differenceKg >= 0 ? 'Implied wastage' : 'Over by'}: <strong>{(Math.abs(balance.differenceKg) / 100).toFixed(3)} qtl</strong>
          {hasInputErrors && <span className="process-field-error" style={{ marginLeft: 12 }}>Fix input quantities above the available lot amount.</span>}
          <Button type="button" style={{ float: 'right' }} disabled={posting || !inputs.length || balance.differenceKg < 0 || hasInputErrors} onClick={() => void postRun()}>{posting ? 'Posting…' : 'Post run'}</Button>
        </div>
      )}
