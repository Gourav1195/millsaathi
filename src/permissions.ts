...
export function shapeOverviewPayload(user: SessionUser, body: Record<string, unknown>): Record<string, unknown> {
  ...
  if (role === 'gate_operator') {
    return applyFinancePolicy(user, {
      me,
      mill: { id: (body.mill as Record<string, unknown>)?.id, name: (body.mill as Record<string, unknown>)?.name, season_label: (body.mill as Record<string, unknown>)?.season_label },
      today: body.today,
      gate: body.gate,
